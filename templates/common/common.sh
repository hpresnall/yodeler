...
rootinstall chrony.conf /etc/chrony
...
rootinstall hosts /etc
rootinstall resolv.conf.head /etc
rootinstall interfaces /etc/network

# use Debian's better ifup / if down
# Busybox's udhcpc does not work with this, so remove it
# link dhclient to a location ifup can find it
rm /sbin/udhcpc
ln -s /usr/sbin/dhclient /sbin/dhclient
rootinstall dhclient.conf /etc/dhcp
